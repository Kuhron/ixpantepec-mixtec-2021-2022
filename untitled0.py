...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 30 13:16:59 2021

@author: emmacmazzuchi
"""
...
if __name__ == "__main__":
    ...
    intervals_hi = [x for x in intervals]
    fmeans = [f0.fmean for interval in intervals_hi for f0 in interval.f0_statistics if f0.fmean is not None and f0.fmean < 300 and f0.fmean > 120]
    fmeanslog = [get_midi_note_from_hz(x) for x in fmeans]
    #print(fmeans)
    hist = plt.hist(fmeans, bins=200)

    intervals_hi = [x for x in intervals if x.glottalized]
    fmeans = [f0.fmean for interval in intervals_hi for f0 in interval.f0_statistics if f0.fmean is not None and f0.fmean < 300 and f0.fmean > 120]
    fmeanslog = [get_midi_note_from_hz(x) for x in fmeans]
    #print(fmeans)
    hist = plt.hist(fmeans, bins=200)
    #add_mean_and_std_lines(plt.gca(), fmeans, hist)
   # std_high = np.mean(fmeans)
   # plt.show()
    
   
    sys.exit()
    
    intervals_lo = [x for x in intervals if x.toneme == "L"]
    fmeans = [f0.fmean for interval in intervals_lo for f0 in interval.f0_statistics if f0.fmean is not None and f0.fmean < 300 and f0.fmean > 120]
    fmeanslog = [get_midi_note_from_hz(x) for x in fmeans]
    print(fmeans)
    hist = plt.hist(fmeans, bins=200)
    add_mean_and_std_lines(plt.gca(), fmeans, hist)
   ...
   # plt.show()
    ...
    intervals_all = [x for x in intervals]
    fmeans = [f0.fmean for interval in intervals_all for f0 in interval.f0_statistics if f0.fmean is not None and f0.fmean < 300 and f0.fmean > 120]
    fmeanslog = [get_midi_note_from_hz(x) for x in fmeans]
    #print(fmeans)
    hist = plt.hist(fmeans, bins=200)
    add_mean_and_std_lines(plt.gca(), fmeans, hist)
    #plt.show()
   # std_all = np.mean(fmeans)
    
    
    
    intervals_HM = [x for x in intervals if x.toneme == "MH"]
    fmaxs = [f0.fmax for interval in intervals_HM for f0 in interval.f0_statistics if f0.fmin is not None and f0.fmean < 350 and f0.fmean > 120]
    fmaxslog = [get_midi_note_from_hz(x) for x in fmaxs]
    #print(fmeans)
    hist = plt.hist(fmaxs, bins=100)
    add_mean_and_std_lines(plt.gca(), fmaxs, hist)
    #plt.show()
    mean_HM = np.mean(fmaxs)
    
    
    intervals_ML = [x for x in intervals if x.toneme == "LM"]
    fmaxs = [f0.fmax for interval in intervals_ML for f0 in interval.f0_statistics if f0.fmin is not None and f0.fmax < 350 and f0.fmean > 120]
    fminslog = [get_midi_note_from_hz(x) for x in fmaxs]
    #print(fmeans)
    hist = plt.hist(fmaxs, bins=100)
    add_mean_and_std_lines(plt.gca(), fmaxs, hist)
    #plt.show()
    mean_ML = np.mean(fmaxs)
    
    
    intervals_HL = [x for x in intervals if x.toneme == "LH"]
    fmaxs = [f0.fmax for interval in intervals_HL for f0 in interval.f0_statistics if f0.fmax is not None and f0.fmax < 350 and f0.fmean > 120]
    fminslog = [get_midi_note_from_hz(x) for x in fmaxs]
    #print(fmeans)
    hist = plt.hist(fmaxs, bins=100)
    add_mean_and_std_lines(plt.gca(), fmaxs, hist)
    plt.show()
    mean_LH = np.mean(fmaxs)
    
   # intervals_mid = [x for x in intervals if x.toneme == "M"]
   # fmeans = [f0.fmean for interval in intervals_mid for f0 in interval.f0_statistics if f0.fmean is not None and f0.fmean < 300 and f0.fmean > 120]
   # fmeanslog = [get_midi_note_from_hz(x) for x in fmeans]
    #print(fmeans)
   # hist = plt.hist(fmeans, bins=200)
   # add_mean_and_std_lines(plt.gca(), fmeans, hist)
    # plt.show()
    #std_mid = np.std(fmeans)
    
    # intervals_hi = [x for x in intervals if x.toneme == "H"]
     #fmeans = [f0.fmean for interval in intervals_hi for f0 in interval.f0_statistics if f0.fmean is not None and f0.fmean < 300 and f0.fmean > 120]
     #fmeanslog = [get_midi_note_from_hz(x) for x in fmeans]
     #print(fmeans)
    # hist = plt.hist(fmeans, bins=200)
   ...
   # print(f"std of all is {std_all}")
    #print(f"std of high is {std_high}")
    #print(f"std of low is {std_low}")
    print(f"mean of HM is {mean_HM}")
    print(f"mean of ML is {mean_ML}")
    print(f"mean of LH is {mean_LH}")
    
    sys.exit()
    
    intervals_hi_log = [x for x in intervals if x.toneme == "M"]
    fmeans = [f0.fmean for interval in intervals_hi_log for f0 in interval.f0_statistics if f0.fmean is not None and f0.fmean < 300 and f0.fmean > 120]
    fmeanslog = [get_midi_note_from_hz(x) for x in fmeans]
    print(fmeanslog)
    plt.hist(fmeanslog, bins=200)
    #plt.show()
    
    intervals_high_preceded_by_low = [interval for previous_interval, interval in zip(intervals[:-1], intervals[1:]) if previous_interval.toneme == "L" and interval.toneme == "M"]
    fmeans = [f0.fmean for interval in intervals_high_preceded_by_low for f0 in interval.f0_statistics if f0.fmean is not None and f0.fmean < 300]
    #print(fmeans)
    weights = np.ones_like(fmeans)/float(len(fmeans))
   # plt.hist(fmeans, bins=75, color="r", label="H / L_", alpha=0.5)
    
    import statistics
    
    intervals_high_not_preceded_by_low = [interval for previous_interval, interval in zip(intervals[:-1], intervals[1:]) if previous_interval.toneme != "M" and interval.toneme == "H"]
   ...
   # print(statistics.mean(fmeans))
   # print(statistics.stdev(fmeans))
    weights = np.ones_like(fmeans)/float(len(fmeans))
    #plt.hist(fmeans, bins=100, color="b", label="H / [MH]_", alpha=0.5, weights=weights)
    #plt.legend()
    #plt.show()
    
    #show mean and stdev in raphs?